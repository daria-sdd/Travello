...
struct AuthBackground: View {
    ...
    /// Центр орбиты самолёта (относительно frame)
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                LinearGradient(
                    colors: [
                        Color(uiColor: UIColor(hex: 0xFAF6F1)),
                        Color(uiColor: UIColor(hex: 0xF4D9B8)),
                    ],
                    startPoint: .top,
                    endPoint: .bottom,
                )
                ...
                Circle()
                    .fill(
                        RadialGradient(
                            colors: [
                                Color(uiColor: UIColor(hex: 0xF4D1A6)).opacity(0.7),
                                Color.clear,
                            ],
                            center: .center,
                            startRadius: 0,
                ...
                // 5. Самолёт по орбите
                Text("✈")
                    .font(.custom("Fraunces-Light", size: 24))
    ...
    private func startAnimations() {
        ...
        // Орбита самолёта — 30 секунд полный круг
        withAnimation(.linear(duration: 30).repeatForever(autoreverses: false)) {
            orbitAngle = 360
        }

        // Покачивание самолёта (для естественности)
        withAnimation(.easeInOut(duration: 7).repeatForever(autoreverses: true)) {
            planePhase = 1.0
        }

        // Облака — каждое по своей скорости
    ...
    private var planeRotation: Double {
        // Поворот по касательной + покачивание
    ...
    private func cloudX(geo: GeometryProxy, phase: Double) -> CGFloat {
        // Облако движется от -100 (за левым краем) до +width+100 (за правым)
...
// ============================================================
// CLOUD SHAPE
// Простое облако — три пересекающихся круга.
// ============================================================
